...
**Re-spread the tier palettes: monotone was not enough** (`PALETTES`/`PALETTE_STEP_MIN` in `src/config.ts`, `CLAUDE.md`, `DESIGN.md`)

Playtest feedback: the per-tier palettes did not read as different tiers. Measuring confirmed it — two tiers out of five were imperceptibly different from the violet they replaced.

- **The defect**: the arc was monotone but lopsided — bottoms 194 -> 261 -> 303 -> 324, steps of 67, 42 and **21** degrees (43, 42 and **7** on the tops). The Dark Wood measured **ΔE 3.9** against the original sky and The Brambles **ΔE 5.0**; below ~2.3 is imperceptible, so those two tiers gained nothing. The existing assertion only checked direction and a 140° cap, so it had nothing to say about spacing.
- **New palettes**, even steps of 52/51/55/53 degrees, with saturation doing the work since luminance is pinned by the halo and witch constraints. Before -> after: The Edge `0x0a141d`/`0x10242a` -> `0x08171f`/`0x0a2629` (sat 0.45 -> 0.60); The Dark Wood `0x090616`/`0x21113e` -> `0x090324`/`0x06084c` (0.57 -> 0.86); The Brambles `0x160718`/`0x341132` -> `0x240321`/`0x3a0545` (0.51 -> 0.86); The Wall `0x080708`/`0x181316` -> `0x0a0808`/`0x151112` (0.12 -> 0.10, pinned dark on purpose); The Moon's Eye fallback `0x190d10`/`0x412327` -> `0x190f0d`/`0x412a23` to keep the authored arc monotone past the Wall. Halos re-spread to one distinct violet each: `0x9d85ff`, `0xaf7aff`, `0xce85ff`, `0xeb99ff`.
- **Measured result** (ΔE, bottoms). Adjacent separation 32.9/12.6/24.2/61.0 -> **51.2/17.0/42.3/61.4**; versus the original sky 33.3/3.9/5.0/14.1 -> **36.6/19.0/19.6/15.7**. Halo contrast improved too: worst 1.159 -> **1.182**, still above the 1.14 floor.
- **New assertion `PALETTE_STEP_MIN` (30°)** on the effective skies, because "one palette per tier" means the tiers must be tellable apart and monotonicity does not deliver that. Verified to reject the arc that shipped, naming the offending pair and the 7° step.
- Two constraints kept the search honest: the halo must stay **violet** (the unconstrained search proposed an orange halo for The Wall, which would read as a reward colour and break both the Affordance rule and the gold rationing), and The Wall stays **pinned dark** — being the blackest tier is half of how it reads, so it keeps the smallest change against the original (ΔE 15.7) and that is correct.
- No gameplay, scoring or difficulty change: rendering only, like `Tier.essence`. `npm run build` green; all four palette assertions verified live.
