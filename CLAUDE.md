...
Categories live in `DEATH_MESSAGES` (i18n.ts), 2-3 variants each, drawn at random. Thresholds live in `DEATH_MESSAGE` (config.ts). They are indexed on **TIME**, like La Percée — that is what lets the line quote the gap in seconds and stay honest. Fixed priority, first match wins: **1.** `newRecord`, the run beat the stored `bestTime` — **2.** `newOmen`, this run's deeds lit a new omen: the ONE event-indexed category (at most twelve firings in a player's life), and it says *that* a sign was earned, **never which** — naming happens on the Scores page alone (-> "The omens") — **3.** `nearRecord`, duration >= `nearRecordRatio` (0.85) of it without beating it, **the only category that may use `{seconds}`** and the only one unreachable without a record — **4.** `bigCombo`, best combo of the run >= `bigComboThreshold` (8) — **5.** `earlyDeath`, the run lasted less than `earlyDeathSeconds` (10 s) — **6.** `default`, anything else.
...
- **No announcement during play, and an omen is only ever NAMED on the page.** NO WORDS DURING PLAY holds, and the death screen stays five things. The reveal is a one-time fade-in with the name in gold for that visit. **The bridge** (playtest amendment -> DESIGN.md, "The bridge") is the closed list of pointers allowed outside the page, and both say only *that* a sign waits, never which: **1.** the `newOmen` death-line category (see "The death screen's one line" — it uses the existing one line, never a sixth element); **2.** the gold glint beside the home screen's Scores label while a lit omen is unrevealed, plus the Scores page opening on the Omens tab in that state. Nothing else — no popup, no counter, no list of earned omens anywhere.
...
- **The hue arc advances in ONE direction** across the five tiers (~208 -> 345 on the tops, ~194 -> 352 on the bottoms, then on to the inversion's pale gold). Tier changes interpolate the sky in RGB over 2 s, and a palette that doubled back would drag the gradient through a muddy neutral. A dev assertion enforces the direction and caps each step at 140°.
- **Luminance stays roughly constant tier to tier.** The Edge reads "luminous" through its HUE, not by being brighter: a brighter Edge was measured and rejected — it put the sky on the witch's own luminance and dropped the halo to a contrast of 1.017.
